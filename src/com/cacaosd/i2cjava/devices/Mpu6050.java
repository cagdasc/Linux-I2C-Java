...
public class Mpu6050 extends I2C {
	...
	public void setSleepMode(boolean mode) {
		byte var = readByteNoExit(Mpu6050Helper.PWR_MGMT_1);
		if (mode) {
			var |= 64;
		} else {
			var &= ~64;
		}
		writeByteNoExit(Mpu6050Helper.PWR_MGMT_1, var);
	}
	...
	public void setRangeAcceleration(byte range) {
		byte afs = readByteNoExit(Mpu6050Helper.ACCEL_CONFIG);
		if (range == 0) {
			afs &= ~24;
		} else if (range == 1) {
			afs |= 8;
		...
		writeByteNoExit(Mpu6050Helper.ACCEL_CONFIG, afs);
	}
	...
	public void setRangeGyroscope(byte range) {
		byte fs = readByteNoExit(Mpu6050Helper.GYRO_CONFIG);
		if (range == 0) {
			fs &= ~24;
		} else if (range == 1) {
			fs |= 8;
		...
		writeByteNoExit(Mpu6050Helper.GYRO_CONFIG, fs);
	}
	...
	public short[] getAccelerations() {
		...
		accels[0] = readWordNoExit(Mpu6050Helper.ACCEL_XOUT_H,
				Mpu6050Helper.ACCEL_XOUT_L);
		accels[1] = readWordNoExit(Mpu6050Helper.ACCEL_YOUT_H,
				Mpu6050Helper.ACCEL_YOUT_L);
		accels[2] = readWordNoExit(Mpu6050Helper.ACCEL_ZOUT_H,
				Mpu6050Helper.ACCEL_ZOUT_L);
		return accels;
	}

	public short getAccelerationX() {
		return readWordNoExit(Mpu6050Helper.ACCEL_XOUT_H,
				Mpu6050Helper.ACCEL_XOUT_L);
	}

	public short getAccelerationY() {
		return readWordNoExit(Mpu6050Helper.ACCEL_YOUT_H,
				Mpu6050Helper.ACCEL_YOUT_L);
	}

	public short getAccelerationZ() {
		return readWordNoExit(Mpu6050Helper.ACCEL_ZOUT_H,
				Mpu6050Helper.ACCEL_ZOUT_L);
	}

	public short[] getAngularVelocities() {
		short[] velocities = new short[3];
		velocities[0] = readWordNoExit(Mpu6050Helper.GYRO_XOUT_H,
				Mpu6050Helper.GYRO_XOUT_L);
		velocities[1] = readWordNoExit(Mpu6050Helper.GYRO_YOUT_H,
				Mpu6050Helper.GYRO_YOUT_L);
		velocities[2] = readWordNoExit(Mpu6050Helper.GYRO_ZOUT_H,
				Mpu6050Helper.GYRO_ZOUT_L);
		return velocities;
	}

	public short getAngularVelocityX() {
		return readWordNoExit(Mpu6050Helper.GYRO_XOUT_H,
				Mpu6050Helper.GYRO_XOUT_L);
	}

	public short getAngularVelocityY() {
		return readWordNoExit(Mpu6050Helper.GYRO_YOUT_H,
				Mpu6050Helper.GYRO_YOUT_L);
	}

	public short getAngularVelocityZ() {
		return readWordNoExit(Mpu6050Helper.GYRO_ZOUT_H,
				Mpu6050Helper.GYRO_ZOUT_L);
	}

	public short getTemperature() {
		return readWordNoExit(Mpu6050Helper.TEMP_OUT_H,
				Mpu6050Helper.TEMP_OUT_L);
	}

	public short[] getMotion6() {
		short[] motion6 = new short[6];
		motion6[0] = readWordNoExit(Mpu6050Helper.ACCEL_XOUT_H,
				Mpu6050Helper.ACCEL_XOUT_L);
		motion6[1] = readWordNoExit(Mpu6050Helper.ACCEL_YOUT_H,
				Mpu6050Helper.ACCEL_YOUT_L);
		motion6[2] = readWordNoExit(Mpu6050Helper.ACCEL_ZOUT_H,
				Mpu6050Helper.ACCEL_ZOUT_L);
		motion6[3] = readWordNoExit(Mpu6050Helper.GYRO_XOUT_H,
				Mpu6050Helper.GYRO_XOUT_L);
		motion6[4] = readWordNoExit(Mpu6050Helper.GYRO_YOUT_H,
				Mpu6050Helper.GYRO_YOUT_L);
		motion6[5] = readWordNoExit(Mpu6050Helper.GYRO_ZOUT_H,
				Mpu6050Helper.GYRO_ZOUT_L);
		return motion6;
	}

	public void setDLPFMode(byte mode) {
		byte config = readByteNoExit(Mpu6050Helper.CONFIG);
		if (mode == 0) {
			config &= ~7;
		} else if (mode == 1) {
			config |= 1;
		...
		writeByteNoExit(Mpu6050Helper.CONFIG, config);
	}
	...
	public void setSampleRate(byte rate) {
		writeByteNoExit(Mpu6050Helper.SMPLRT_DIV, rate);
	}

	public byte getSampleRate() {
		return readByteNoExit(Mpu6050Helper.SMPLRT_DIV);
	}

	public void setMotionDetectionThresold(byte value) {
		writeByteNoExit(Mpu6050Helper.MOT_THR, value);
	}

	public byte getMotionDecetionThresold() {
		return readByteNoExit(Mpu6050Helper.MOT_THR);
	}

	public void setTEMP_FIFO_EN(byte value) {
		writeBitNoExit(Mpu6050Helper.FIFO_EN, value,
				Mpu6050Helper.TEMP_FIFO_EN_BIT);
	}

	public byte getTEMP_FIFO_EN() {
		return readBitNoExit(Mpu6050Helper.FIFO_EN,
				Mpu6050Helper.TEMP_FIFO_EN_BIT);
	}
	...
	public short getFIFO_Count() {
		return readWordNoExit(Mpu6050Helper.FIFO_COUNTH,
				Mpu6050Helper.FIFO_COUNTL);
	}

	public void setFIFO_Enable(byte value) {
		writeBitNoExit(Mpu6050Helper.USER_CTRL, value,
				Mpu6050Helper.FIFO_EN_BIT);
	}

	public byte getFIFO_Enable() {
		return readBitNoExit(Mpu6050Helper.USER_CTRL, Mpu6050Helper.FIFO_EN_BIT);
	}

	public byte[] getFIFO_Data(byte length) {
		return readByteBufferNoExit(Mpu6050Helper.FIFO_R_W, length);
	}
	...
	public byte getFIFO_Reset() {
		return readBitNoExit(Mpu6050Helper.USER_CTRL,
				Mpu6050Helper.FIFO_RESET_BIT);
	}
}
